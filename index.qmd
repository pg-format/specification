...
a [node](#nodes) or an [edge](#edges). Statements are separated from each other
with a [line break](#whitespace).^[As [discussed
here](https://github.com/orgs/pg-format/discussions/22) an additional explicit
statement separator may also be used.] [Empty lines](#whitespace) between
...
A **edge** consists of the following elements, given in this order and separated by [whitespace]:

- an OPTIONAL **edge identifier** given as [identifier] immediately followed by a colon
...
Labels of a node or edge are a sorted set.

~~~{.pg #lst-labels-repeated lst-cap="Repeated labels on same node or edge are ignored}
...
**Spaces** is a non-empty sequence of space (`U+0020`) and/or tabular (`U+0009`).

A **comment** begins with a hash (`#` = `U+0023`) and ends after the next line
break or at the end of the document.

An **empty line** begins after a line break or at the start of the document. It
ends with a comment, after the next line break, or at the end of the document
and it contains nothing but optional space in between.

Mandatory **whitespace** separates elements of a statement. Whitespace consists of
a non-empty sequence of any of space and/or comments.

If a non-empty line begins with Spaces it is considered a continuation of the
previous statement.
