...
from each other with a [line break](#whitespace). Optional
...
apostrophe are allowed in an unquoted identifier after its first character.
...
A label is a an [identifier] following a colon (`U+003A`).
...
- OPTIONAL [spaces](#whitespace)
- a colon (`U+003A`)
- a non-empty list of **[property values](#property-values)**, separated by comma (`U+002C`)

A property value MAY further be preceded and followed by [delimiting whitespace](#whitespace).
...
a key : value       # optional spaces around colon
a key :value        # allowed but not recommended (value looks like label)
